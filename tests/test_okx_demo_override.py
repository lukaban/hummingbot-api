...
class OkxDemoOverrideTest(unittest.TestCase):
    def test_demo_domain_is_registered_and_simulated_header_is_forced(self):
        ...
        self.assertIn('environment["HTTP_PROXY"] = ""', docker_service)
        self.assertIn('environment["HTTPS_PROXY"] = ""', docker_service)
        self.assertIn('environment["NO_PROXY"] = "*"', docker_service)
        self.assertNotIn('"www.okx.com": "104.18.43.174"', docker_service)
        ...
        self.assertIn('body_params = {"instType": "SWAP", "limit": 100}', derivative)
        self.assertIn('"mgnMode": "isolated"', derivative)
        self.assertIn("exchange_position_mode = await self._fetch_account_position_mode()", derivative)
        self.assertIn('exchange_position_mode == PositionMode.HEDGE', derivative)
        self.assertIn('"code":"51603"', derivative)
        self.assertIn("async def _handle_update_error_for_active_order", derivative)
        self.assertIn("keeping it tracked for retry", derivative)
        self.assertIn("def _normalized_order_state", derivative)
        self.assertIn("accepted_size < requested_size", derivative)
        self.assertIn("return OrderState.CANCELED", derivative)
        self.assertNotIn("trade_history_tasks", derivative)
        self.assertNotIn("self._last_trade_history_timestamp = max(timestamps) if timestamps else None", derivative)
        ast.parse(utils)
