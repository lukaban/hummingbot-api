...
class DemoSafeOkxPerpetualAPIOrderBookDataSource(OkxPerpetualAPIOrderBookDataSource):
    async def _set_trading_rules(self) -> Dict[str, Any]:
        if self._domain != CONSTANTS.DEMO_DOMAIN:
            return await super()._set_trading_rules()
        if not self._trading_rules:
            response = await self._request_trading_rules_info()
            self._trading_rules = {
                rule["instId"]: float(rule["ctVal"])
                for rule in response["data"] if rule.get("ctVal")
            }
...
class OkxPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def _is_request_exception_related_to_time_synchronizer(self, request_exception: Exception):
        ...
        is_time_synchronizer_related = any(f'"code":"{code}"' in error_description for code in ("50102", "50113"))
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return DemoSafeOkxPerpetualAPIOrderBookDataSource(
            self.trading_pairs,
            connector=self,
    ...
    async def _api_request(self,
        ...
        if self._domain == CONSTANTS.DEMO_DOMAIN:
            headers = {**(headers or {}), "x-simulated-trading": "1"}
        ...
        for _ in range(2):
            try:
                ...
                if self._is_request_exception_related_to_time_synchronizer(request_exception):
